...
public class AdbTvClientFactory(ILogger<AdbTvClientFactory> logger)
{
    ...
    public async ValueTask<DeviceClient?> TryGetOrCreateClientAsync(AdbTvClientKey adbTvClientKey, CancellationToken cancellationToken)
    {
        if (!_clients.TryGetValue(adbTvClientKey, out var deviceClientHolder))
        {
            if (await _globalSemaphore.WaitAsync(TimeSpan.FromSeconds(3), cancellationToken))
            {
                try
                {
                    return await CreateDeviceClient(adbTvClientKey, cancellationToken);
                }
                finally
                {
    ...
    private async ValueTask<DeviceClient?> CreateDeviceClient(AdbTvClientKey adbTvClientKey, CancellationToken cancellationToken)
    {
        ...
        _clients[adbTvClientKey] = new DeviceClientHolder(deviceClient, new SemaphoreSlim(1, 1));
        return deviceClient;
    }
